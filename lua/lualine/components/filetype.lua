...
function FileType:apply_icon()
	...
	if ok then
		...
		if icon and self.options.colored then
			...
			local default_highlight = highlight.format_highlight(is_focused, self.options.self.section)
			...
			icon = highlight.component_format_highlight(icon_highlight) .. icon .. default_highlight
		end
	else
